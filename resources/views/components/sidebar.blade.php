            ...
            <div>
                ...
                <ul class="mb-6 flex flex-col gap-4">
                    ...
                    @if (auth()->user()->role === 'admin')
                        ...
                        <li x-data="{
                            selected: '',
                            isActive: {{ request()->routeIs('users.*') || request()->routeIs('opas.*') ? 'true' : 'false' }}
                        }">
                            <a href="#" @click.prevent="selected = (selected === 'Anggota' ? '' : 'Anggota')"
                                ...
                                <!-- ICON -->
                                @if (request()->routeIs('users.*') || request()->routeIs('opas.*'))
                                    <!-- Aktif -->
                            ...
                            <div class="translate transform overflow-hidden"
                                :class="(selected === 'Anggota' || isActive) ? 'block' : 'hidden'">
                        ...
                        <li x-data="{
                            ...
                            <a href="#" @click.prevent="selected = (selected === 'Kegiatan' ? '' : 'Kegiatan')"
                                ...
                                @else
                                    {{-- Tidak aktif --}}
                                    <svg class="menu-item-icon ms-1" width="24" height="24"
                    ...
                    <li x-data="{
                        selected: '',
                        isActive: {{ in_array(Route::currentRouteName(), ['loans.index', 'loan-details.index', 'loans.manage', 'loans.show', 'notifications.show']) ? 'true' : 'false' }}
                    }">
                        ...
                        <div class="overflow-hidden transition-all duration-300"
                            ...
                            <ul class="mt-2 flex flex-col gap-1 pl-9" :class="sidebarToggle ? 'lg:hidden' : 'flex'">
                                <li>
                                    <a href="{{ route('loans.index') }}"
                                        class="menu-dropdown-item group py-3.5 font-normal
                {{ in_array(Route::currentRouteName(), ['loans.index', 'loans.manage', 'loans.show', 'notifications.show'])
                    ? 'menu-dropdown-item-active bg-[#3A1096]/40 text-white dark:text-white'
                    : 'menu-dropdown-item-inactive text-gray-700 dark:text-gray-300' }}">
                                        Daftar Peminjaman
                                    </a>
                                </li>
                    ...
                    @if (auth()->user()->role === 'admin')
                        ...
                        <li>
                            <a href="{{ route('messages.index') }}"
                                class="menu-item font-normal group rounded-lg py-3.5 {{ request()->routeIs('messages.*') ? 'bg-[#3A1096]/50' : 'menu-item-inactive' }}">

                                <!-- Icon -->
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    class="menu-item-icon ms-1 w-6 h-6 transition-colors duration-200
            {{ request()->routeIs('messages.*') ? 'text-white' : 'text-gray-600 dark:text-gray-300' }}"
                                    fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">

                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M21.75 7.5v9A2.25 2.25 0 0119.5 18.75h-15A2.25 2.25 0 012.25 16.5v-9A2.25 2.25 0 014.5 5.25h15A2.25 2.25 0 0121.75 7.5z" />
                                ...
                                <!-- Text -->
                                <span
                                    class="menu-item-text transition-colors duration-200 dark:text-gray-300 {{ request()->routeIs('messages.*') ? 'text-white' : 'text-black' }}"
                                    :class="sidebarToggle ? 'lg:hidden' : ''">
                                    Pesan
                                </span>
